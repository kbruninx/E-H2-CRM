function define_EOM_parameters!(EOM::Dict, data::Dict, ts::Dict, repr_days::Dict)
    ...
    for jy in keys(years)
        # timeseries
        EOM["D"][:,:,jy] = [ts[jy][!,:LOAD][round(Int, data["nTimesteps"]*(repr_days[jy][!,:periods][jd]-1) + jh)]/10^6 for jh=1:data["nTimesteps"], jd=1:data["nReprDays"]] # from MWh to TWh
    
        EOM["elasticity"][:,:,jy] = [ts[jy][!,:ELASTICITY_EL][round(Int, data["nTimesteps"]*(repr_days[jy][!,:periods][jd]-1) + jh)]/10^6 for jh=1:data["nTimesteps"], jd=1:data["nReprDays"]]
        #EOM["elasticity"] = data["elastic_sec_EOM"]
