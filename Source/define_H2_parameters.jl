function define_H2_parameters!(H2::Dict, data::Dict, ts::Dict, repr_days::Dict)
    ...
    for jy in keys(years)
        ...
        H2["D"][:,:,jy] = [ts[jy][!,:LOAD_H2][round(Int, data["nTimesteps"]*(repr_days[jy][!,:periods][jd]-1) + jh)]/10^5 for jh=1:data["nTimesteps"], jd=1:data["nReprDays"]] # 10^2 GWh
        # N.B. to change H2 load in "timeseries.csv", go back to "H2demandincrease" excel sheet
