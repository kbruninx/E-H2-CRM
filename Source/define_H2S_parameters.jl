function define_H2S_parameters!(m::String, mod::Model, data::Dict, repr_days::Dict, order_matrix::Dict)
    ...
    elseif m == "electrolysis"
        ...
        mod.ext[:parameters][:IC] = (data["discount_rate"] * data["OC"]) / (1 - (1 / ((1 + data["discount_rate"])^data["Lifetime"]))) # EUR/MW 
    elseif m == "H2storage"
        ...
        mod.ext[:parameters][:IC_cap] = (data["discount_rate"] * data["OC_cap"]) / (1 - (1 / ((1 + data["discount_rate"])^data["Lifetime"]))) # EUR/MW - costs for installed capacity
        mod.ext[:parameters][:IC_vol] = (data["discount_rate"] * data["OC_vol"]) / (1 - (1 / ((1 + data["discount_rate"])^data["Lifetime"]))) # EUR/MWh - costs for installed volume
