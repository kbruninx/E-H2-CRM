function define_H2S_parameters!(m::String, mod::Model, data::Dict, repr_days::Dict, order_matrix::Dict)
    ...
    elseif m == "electrolysis"
        ...
        mod.ext[:parameters][:IC] = data["OC"]/data["Lifetime"] * (1 + data["inflation"])^data["Lifetime"] / (1 + data["discount_rate"])^data["Lifetime"] # MEUR/GW 
    elseif m == "H2storage"
        ...
        mod.ext[:parameters][:IC_cap] = data["OC_cap"]/data["Lifetime"] * (1 + data["inflation"])^data["Lifetime"] / (1 + data["discount_rate"])^data["Lifetime"] # MEUR/GW - costs for installed capacity
        mod.ext[:parameters][:IC_vol] = data["OC_vol"]/data["Lifetime"] * (1 + data["inflation"])^data["Lifetime"] / (1 + data["discount_rate"])^data["Lifetime"] # MEUR/TWh - costs for installed volume
