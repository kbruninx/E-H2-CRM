function build_ps_agent!(m::String,mod::Model,EOM::Dict)
    ...
    if m == "Edemand"
        ...
        # Create variables
        g_VOLL = mod.ext[:variables][:g_VOLL] = @variable(mod, [jh = JH, jd = JD, jy = JY], upper_bound = 0, base_name = "demand")
        g_ela = mod.ext[:variables][:g_ela] = @variable(mod, [jh = JH, jd = JD, jy = JY], upper_bound = 0, base_name = "elastic demand")
        ...
        if γ < 1
            ...
            α + profit[jy] <= u[jy] )         # constraint adapted to deal with the signs change
        end
    ...
    elseif m == "Biomass"
        ...
        mod.ext[:constraints][:CM] = @constraint(mod,
        cap_cm <= cap )      #  using the same variable for the investment capacity and the cm offered capacity means all the installed capacity is sold on th cm
